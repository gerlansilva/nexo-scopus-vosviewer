...
ACCESS_MARKER_RE = re.compile(
    r"\b(?:acesso\s+em|accessed|consultado\s+em|dispon[ií]vel\s+em|"
    r"available\s+(?:at|from)|recuperado\s+de|retrieved\s+from)\b",
    re.I,
)

REPEATED_AUTHOR_RE = re.compile(r"^[—–―_\-]{2,}\s*[.,;:]?")

INLINE_REFERENCE_START_RE = re.compile(
    r"(?<=\.)\s+(?="
    r"(?:\[\d+\]\s*)?"
    r"(?:[A-ZÀ-ÖØ-Þ][A-ZÀ-ÖØ-Þ'’\-]{1,}(?:\s+(?:DA|DE|DO|DAS|DOS|"
    r"NETO|FILHO|J[ÚU]NIOR))*\s*[,.;]|"
    r"[A-ZÀ-ÖØ-Þ][A-Za-zÀ-ÖØ-öø-ÿ'’\-]+,\s*(?:[A-ZÀ-ÖØ-Þ]\.\s*)+)"
    r")"
)
...
def line_starts_reference(line: str) -> bool:
    ...
    if REPEATED_AUTHOR_RE.match(line):
        return True
    institution = (
    ...
    apa = (
        ...
        r"van|von))?(?:\s+[A-ZÀ-ÖØ-Þ][A-Za-zÀ-ÖØ-öø-ÿ'’\-]+)?,\s*"
    ...
    return bool(
        ...
        or (
            re.match(apa, line)
            and re.search(r"\(?\b(?:18|19|20)\d{2}[a-z]?\b\)?", line[:320], re.I)
        )
    )
...
def segment_references(section: str) -> list[str]:
    ...
    for raw in section.splitlines():
        ...
        if not line or is_noise_line(line):
            continue
        # Alguns conversores de PDF colam duas referências completas na mesma
        # linha. A divisão só é feita após ponto e diante de um padrão de
        # autoria suficientemente forte.
        lines.extend(
            compact(part)
            for part in INLINE_REFERENCE_START_RE.split(line)
            if compact(part)
        )
    ...
    resolved = []
    for block in blocks:
        block = re.sub(r"^\s*(?:\[\d+\]|\d+[.)])\s*", "", block)
        if REPEATED_AUTHOR_RE.match(block):
            if previous_authors:
                block = REPEATED_AUTHOR_RE.sub(f"{previous_authors}. ", block, count=1)
        else:
            candidate_year = extract_year(block)
            candidate_style = detect_style(block)
            candidate = extract_author_block(block, candidate_style, candidate_year)
            if candidate:
                previous_authors = candidate
        if len(block) >= 20:
            resolved.append(compact(block))
    return resolved
...
def extract_year(ref: str) -> str:
    # Ano de acesso e anos contidos em URLs não são anos da obra.
    before_access = ACCESS_MARKER_RE.split(ref, maxsplit=1)[0]
    before_access = re.sub(r"https?://\S+|www\.\S+", "", before_access, flags=re.I)
    apa = re.match(r"^.{1,260}?\(\s*((?:18|19|20)\d{2})[a-z]?\s*\)", before_access)
    if apa:
        return apa.group(1)
    years = YEAR_RE.findall(before_access)
...
def extract_author_block(ref: str, style: str, year: str) -> str:
    ...
    # Em ABNT, pontos de iniciais (R. M.) não encerram a autoria. O título
    # normalmente começa por uma palavra com pelo menos duas letras depois do
    # ponto que fecha o último nome.
    match = re.match(
        r"^(.+?)\.\s+(?=[A-ZÀ-ÖØ-Þ][A-Za-zÀ-ÖØ-öø-ÿ'’\-]{1,}\b)",
        ref,
    )
...
def initials(given: str) -> str:
    ...
    for token in re.findall(r"[A-Za-zÀ-ÖØ-öø-ÿ]+", given):
        if token.casefold() in {"da", "de", "do", "das", "dos", "del", "van", "von"}:
            continue
        if token[:1].isupper():
...
def remove_opening_and_year(ref: str, author_block: str, year: str) -> str:
    ...
    body = remove_urls_and_details(body)
    if year:
        body = re.sub(
            rf"[,.;:\s]*\(?{re.escape(year)}[a-z]?\)?\.?\s*$",
            "",
            body,
            flags=re.I,
        )
    return compact(body).strip(" ,.;")
...
def parse_reference(article_id: str, pdf_file: str, order: int, raw: str) -> Reference:
    ...
    if re.search(r"\bet\s+al\.?", author_block, re.I):
        warnings.append("truncated_authors")
    if not plausible_authors(authors):
        warnings.append("implausible_authors")
    if not plausible_title(title):
        warnings.append("implausible_title")
    if not plausible_source(source):
        warnings.append("implausible_source")
    if possible_joined_references(raw):
        warnings.append("possible_joined_references")
...
def plausible_authors(value: str) -> bool:
    value = compact(value)
    if not value or len(value) > 420 or len(value.split()) > 55:
        return False
    if ACCESS_MARKER_RE.search(value) or YEAR_RE.search(value):
        return False
    forbidden_words = (
        "revista", "journal", "universidade", "volume", "educação",
        "education", "disponível", "http", "anais",
    )
    low = fold(value)
    return not any(word in low for word in forbidden_words)


def plausible_title(value: str) -> bool:
    value = compact(value)
    if len(value) < 4 or len(value) > 650:
        return False
    if ACCESS_MARKER_RE.search(value):
        return False
    return not bool(re.match(r"^(?:https?://|www\.|p\.?\s*\d)", value, re.I))


def plausible_source(value: str) -> bool:
    value = compact(value)
    if len(value) < 2 or len(value) > 480:
        return False
    if ACCESS_MARKER_RE.search(value) or re.search(r"https?://|www\.|\bdoi\b", value, re.I):
        return False
    # Um segundo bloco de autoria dentro da fonte costuma indicar duas obras
    # coladas, ou uma divisão título/fonte incorreta.
    return not bool(
        re.search(
            r"\.\s+[A-ZÀ-ÖØ-Þ][A-ZÀ-ÖØ-Þ'’\-]{2,}\s*,\s*"
            r"(?:[A-ZÀ-ÖØ-Þ]\.\s*){1,4}",
            value,
        )
    )
...
def possible_joined_references(value: str) -> bool:
    value = compact(value)
    starts = INLINE_REFERENCE_START_RE.findall(value)
    access_count = len(ACCESS_MARKER_RE.findall(value))
    return len(starts) > 0 or access_count > 2 or len(YEAR_RE.findall(value)) > 7
...
def validate_review(rows: list[dict[str, str]], strict: bool) -> list[dict[str, str]]:
    ...
    for position, row in enumerate(rows, start=2):
        ...
        if not plausible_authors(authors):
            row_errors.append("implausible_authors")
        if not plausible_title(title):
            row_errors.append("implausible_title")
        if not plausible_source(source):
            row_errors.append("implausible_source")
        original = compact(row.get("reference_original", ""))
        if possible_joined_references(original):
            row_errors.append("possible_joined_references")
        # Não apaga automaticamente o alerta de autoria truncada: se o PDF
        # trouxe "et al.", é indispensável completar os autores na revisão.
        prior_warnings = compact(row.get("warnings", ""))
        if "truncated_authors" in prior_warnings and len(authors.split(",")) < 2:
            row_errors.append("truncated_authors")
