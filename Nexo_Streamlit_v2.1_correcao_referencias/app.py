...
def header() -> None:
    st.markdown(
        ...
        <div class="nexo-header">
          ...
          <div class="nexo-version">PDF → Scopus / VOSviewer · v2.1</div>
        </div>
        """,
        unsafe_allow_html=True,
    )
...
if "review_df" not in st.session_state:
    ...
    if "final_files" not in st.session_state:
        ...
        with top_left:
            ...
            st.caption(
                "Compare com a referência original e edite Autor(es), título, fonte e ano. "
                "Use “no” em Incluir somente quando a linha não for uma referência."
            )
        with top_right:
        ...
        display_columns = [
            "include", "pdf_file", "reference_order", "reference_original",
            "authors", "title", "source", "year", "status", "warnings",
        ]
        edited = st.data_editor(
            ...
            height=520,
            disabled=[
                "pdf_file", "reference_order", "reference_original",
                "status", "warnings",
            ],
            column_config={
                ...
                "reference_original": st.column_config.TextColumn(
                    "Referência original", width="large"
                ),
        ...
        if remaining:
            st.warning(
                ...
                "Os avisos indicam campo ausente, autoria truncada, conteúdo "
                "implausível ou possível união de obras."
            )
        else:
